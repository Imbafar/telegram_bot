...
def get_new_image_baby():
    ...
    URL = f'https://pixabay.com/api/?key={PIXABAY}&q=новорожденный&image_type=photo&lang=ru&page={random_page}'
    try:
        response = requests.get(URL).json().get('hits')[random_val].get('webformatURL')
    except Exception as error:
        logging.error(f'Ошибка при запросе к основному API: {error}')
    return response
